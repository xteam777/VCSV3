...
//  ipBase_Screen, ipBase_DirtyArray, ipBase_MovedArray: PByte; //Àäðåñ MMF, ïåðåäàâàåìûé èç êëèåíòà
  hOld: HGDIOBJ;
  CurrentPID: DWORD;
  HeaderSize: Integer;
  err: LongInt;
...
{    if (Win32MajorVersion >= 6) then //vista\server 2k8
    begin
      ...
      begin
        file_name1 := GetTempFile + '.exe'; //Äîäåëàòü ÷åðåç SAS
...
var
  ...
  BitMap, Buf: HBitMap;
begin
  DesktopHandle := GetDC(GetDesktopWindow);//Handle ýêðàíà
  //êîïèÿ ýêðàíà
  BitMapHandle := CreateCompatibleDC(GetDC(0));//ñîçäàíèå ñîâìåñòèìîãî handl-à
  BitMap := CreateCompatibleBitmap(GetDC(0),GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN));//ñîçäàíèå ñîâìåñòèìîé áèòîâîé êàðòû
  SelectObject(BitMapHandle, BitMap);//ïðèìåíåíèå
  BitBlt(BitMapHandle,//êîïèðîâàíèå ýêðàíà
  0,0,GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
  DesktopHandle,0,0,
  SRCCOPY);

  //áóôåð (÷òîáû èçáàâèòüñÿ îò ìåðöàíèÿ)
  BufHandle := CreateCompatibleDC(GetDC(0));//ñîçäàíèå ñîâìåñòèìîãî handl-à
  buf := CreateCompatibleBitmap(GetDC(0),GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN));//ñîçäàíèå ñîâìåñòèìîé áèòîâîé êàðòû
  SelectObject(BufHandle, Buf);//ïðèìåíåíèå

  DeleteDC(DesktopHandle);//óäàëåíèå Handl-à ýêðàíà

  BitBlt(BufHandle, //êîïèðîâàíèå ñíèìêà ýêðàíà â áóôåð
  0,0,GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
  BitMapHandle,0,0,
  SRCCOPY);
  ...
  BitBlt(SaveBitMap.Canvas.Handle, //êóäà
  0,0,sWidth,sHeight,//êîîðäèíàòû è ðàçìåð
  BufHandle, //îòêóäà
  0,0, //êîîðäèíàòû
  SRCCOPY); //ðåæèì êîïèðîâàíèÿ
...
begin
  ...
  begin
    ...
    biPlanes := 1;
    biBitCount := GetDeviceCaps(hScrDC, BITSPIXEL); //Будет обновлено после чтения данных от клиента
...
//Â õåëïåðå ñîçäàåòñÿ îòîáðàæåíèå â ïàìÿòü è çàïîëíÿåòñÿ
...
//  //Èíèöèàëèçèðóåì âõîäíûå ïàðàìåòðû
//  HelperIOData.PID := 0;
...
//  CurOffset := CurOffset + sizeof(ipBase_Screen); //Àäðåñ ïåðåìåííîé, ñ âûäåëåííîé ïàìÿòüþ â ïðîöåññå êëèåíòà äëÿ çàïèñè ñêðèíøîòà
...
//  time := GetTickCount;
  ...
  //Çàïèñûâàåì äàííûå â ïðîöåññ

  hProc := OpenProcess(PROCESS_VM_WRITE or PROCESS_VM_OPERATION, False, HelperIOData.PID); // ïîäêëþ÷àåìñÿ ê ïðîöåññó çíàÿ åãî ID
  if hProc <> 0 then // óñëîâèå ïðîâåðêè ïîäêëþ÷åíèÿ ê ïðîöåññó
  try
    WriteProcessMemory(hProc, HelperIOData.ipBase_ScreenBuff, FDesktopDuplicator.ScreenBuff, HelperIOData.BitmapSize, numberWrite); // çàïèñü ñêðèíøîòà â ïàìÿòü ïðîöåññà
    ...
    CloseHandle(hProc); // îòñîåäèíÿåìñÿ îò ïðîöåññà
  end;
