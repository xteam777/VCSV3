...
This unit is a merging of the original TvCaptionButton, TJvaCaptionButton.
...
type
  ...
  TvCaptionButtonLayout = (cbImageLeft, cbImageRight);
  TJvRedrawKind = (rkDirect, rkIndirect, rkTotalCaptionBar);

  TvCaptionButton = class;

  TvCaptionButtonActionLink = class(TActionLink)
  protected
    FClient: TvCaptionButton;
  ...
  TvCaptionButtonActionLinkClass = class of TvCaptionButtonActionLink;

  {$IFDEF RTL230_UP}
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF RTL230_UP}
  TvCaptionButton = class(TJvComponent)
  private
    { Properties }
    ...
    FActionLink: TvCaptionButtonActionLink;
    FBuffer: TBitmap;
    FButtonRect: TRect;
    ...
    procedure SetLayout(const Value: TvCaptionButtonLayout);
  ...
  protected
    ...
    function GetActionLinkClass: TvCaptionButtonActionLinkClass; dynamic;
    ...
    property ActionLink: TvCaptionButtonActionLink read FActionLink write FActionLink;
  ...
  published
    ...
    property Layout: TvCaptionButtonLayout read FLayout write SetLayout default cbImageLeft;
...
procedure Register;
begin
  RegisterComponents('Samples', [TvCaptionButton]);
end;
...
constructor TvCaptionButton.Create(AOwner: TComponent);
begin
...
destructor TvCaptionButton.Destroy;
begin
  DestroyToolTip;
...
procedure TvCaptionButton.ActionChange(Sender: TObject;
...
procedure TvCaptionButton.Assign(Source: TPersistent);
begin
  if Source is TvCaptionButton then
  begin
    Alignment := TvCaptionButton(Source).Alignment;
    ButtonHeight := TvCaptionButton(Source).ButtonHeight;
    ButtonLeft := TvCaptionButton(Source).ButtonLeft;
    ButtonTop := TvCaptionButton(Source).ButtonTop;
    ButtonWidth := TvCaptionButton(Source).ButtonWidth;
    Caption := TvCaptionButton(Source).Caption;
    ShowHint := TvCaptionButton(Source).ShowHint;
    ParentShowHint := TvCaptionButton(Source).ParentShowHint;
    Enabled := TvCaptionButton(Source).Enabled;
    Font := TvCaptionButton(Source).Font;
    Hint := TvCaptionButton(Source).Hint;
    ImageIndex := TvCaptionButton(Source).ImageIndex;
    Images := TvCaptionButton(Source).Images;
    Layout := TvCaptionButton(Source).Layout;
    Margin := TvCaptionButton(Source).Margin;
    Position := TvCaptionButton(Source).Position;
    Spacing := TvCaptionButton(Source).Spacing;
    Standard := TvCaptionButton(Source).Standard;
    // set toggle before down
    Toggle := TvCaptionButton(Source).Toggle;
    Down := TvCaptionButton(Source).Down;
    Visible := TvCaptionButton(Source).Visible;
  end
  else
    inherited Assign(Source);
end;

procedure TvCaptionButton.CalcButtonParts(ACanvas: TCanvas;
...
procedure TvCaptionButton.CalcDefaultButtonRect(Wnd: THandle);
const
...
begin
  ...
  FHasSmallCaption := True; //ExStyle and WS_EX_TOOLWINDOW = WS_EX_TOOLWINDOW;
  {$IFDEF JVCLThemesEnabled}
...
procedure TvCaptionButton.Click;
begin
...
procedure TvCaptionButton.CreateToolTip(Wnd: THandle);
var
  ToolInfo: TToolInfo;
begin
  if csDesigning in ComponentState then
...
procedure TvCaptionButton.DestroyToolTip;
begin
  if FToolTipHandle <> 0 then
  begin
...
procedure TvCaptionButton.DoActionChange(Sender: TObject);
begin
  if Sender = Action then
    ActionChange(Sender, False);
end;

procedure TvCaptionButton.DrawButton(DC: HDC);
var
...
{$IFDEF JVCLThemesEnabled}
procedure TvCaptionButton.DrawButtonBackground(ACanvas: TCanvas);
const
...
procedure TvCaptionButton.DrawButtonImage(ACanvas: TCanvas; ImageBounds: TRect);
begin
...
procedure TvCaptionButton.DrawButtonText(ACanvas: TCanvas; TextBounds: TRect);
var
  Flags: DWORD;
  OldFont: TFont;
begin
...
procedure TvCaptionButton.DrawNonStandardButton(ACanvas: TCanvas);
{$IFDEF JVCLThemesEnabled}
const
  cState_Normal = 1;
  cState_Hot = 2;
...
procedure TvCaptionButton.DrawStandardButton(ACanvas: TCanvas);
const
  {$IFDEF JVCLThemesEnabled}
...
begin
  ...
  if IsThemed then
  begin
    CaptionButton := twMinButtonNormal;//CElements[FStandard];
...
procedure TvCaptionButton.ForwardToToolTip(Msg: TMessage);
var
  ForwardMsg: TMsg;
begin
  if FToolTipHandle = 0 then
...
function TvCaptionButton.GetAction: TBasicAction;
begin
...
function TvCaptionButton.GetActionLinkClass: TvCaptionButtonActionLinkClass;
begin
  Result := TvCaptionButtonActionLink;
end;

function TvCaptionButton.GetIsImageVisible: Boolean;
begin
  Result := Assigned(Images) and (ImageIndex > -1) and (ImageIndex < Images.Count);
end;

{$IFDEF JVCLThemesEnabled}
function TvCaptionButton.GetIsThemed: Boolean;
begin
  Result := GlobalXPData.IsThemed;
end;
{$ENDIF JVCLThemesEnabled}

function TvCaptionButton.GetParentForm: TCustomForm;
begin
...
function TvCaptionButton.GetParentFormHandle: THandle;
var
  P: TCustomForm;
begin
  P := GetParentForm;
...
function TvCaptionButton.HandleButtonDown(var Msg: TWMNCHitMessage): Boolean;
begin
...
function TvCaptionButton.HandleButtonUp(var Msg: TWMNCHitMessage): Boolean;
var
  DoClick: Boolean;
  P: TPoint;
begin
...
function TvCaptionButton.HandleHitTest(var Msg: TWMNCHitTest): Boolean;
var
  CurPos: TPoint;
begin
...
function TvCaptionButton.HandleMouseMove(var Msg: TWMNCHitMessage): Boolean;
var
  DoRedraw: Boolean;
...
procedure TvCaptionButton.HandleNCActivate(var Msg: TWMNCActivate);
begin
  {$IFDEF JVCLThemesEnabled}
...
procedure TvCaptionButton.HandleNCMouseMove(var Msg: TWMNCHitMessage);
var
  IsOnButton: Boolean;
begin
...
procedure TvCaptionButton.HandleNCPaintAfter(Wnd: THandle; var Msg: TWMNCPaint);
begin
  if FRgnChanged then
  begin
...
procedure TvCaptionButton.HandleNCPaintBefore(Wnd: THandle; var Msg: TWMNCPaint);
var
  WindowRect: TRect;
  DrawRgn: HRGN;
  LButtonRect: TRect;
...
function TvCaptionButton.HandleNotify(var Msg: TWMNotify): Boolean;
var
  CurPos: TPoint;
...
procedure TvCaptionButton.HideToolTip;
begin
  if FToolTipHandle <> 0 then
    SendMessage(FToolTipHandle, TTM_POP, 0, 0);
end;

procedure TvCaptionButton.Hook;
var
  P: TCustomForm;
begin
...
procedure TvCaptionButton.ImageListChange(Sender: TObject);
begin
  if Sender = Images then
    Redraw(rkIndirect);
end;

procedure TvCaptionButton.InitiateAction;
begin
  if FActionLink <> nil then
    FActionLink.Update;
end;

function TvCaptionButton.IsCaptionStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsCaptionLinked;
end;

function TvCaptionButton.IsEnabledStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsEnabledLinked;
end;

function TvCaptionButton.IsHintStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsHintLinked;
end;

function TvCaptionButton.IsImageIndexStored: Boolean;
begin
  Result := (ActionLink = nil) or not FActionLink.IsImageIndexLinked;
end;

procedure TvCaptionButton.Loaded;
begin
  inherited Loaded;
...
procedure TvCaptionButton.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseDown) then
    FOnMouseDown(Self, Button, Shift, X, Y);
end;

procedure TvCaptionButton.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseMove) then
    FOnMouseMove(Self, Shift, X, Y);
end;

function TvCaptionButton.MouseOnButton(X, Y: Integer;
...
procedure TvCaptionButton.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if Assigned(FOnMouseUp) then
    FOnMouseUp(Self, Button, Shift, X, Y);
end;

procedure TvCaptionButton.Notification(AComponent: TComponent;
...
procedure TvCaptionButton.Redraw(const AKind: TJvRedrawKind);
var
  Wnd: THandle;
  DC: HDC;
begin
...
procedure TvCaptionButton.ResetButton;
begin
  UnHook;
  Hook;
  Redraw(rkTotalCaptionBar);
...
procedure TvCaptionButton.SetAction(const Value: TBasicAction);
begin
...
procedure TvCaptionButton.SetAlignment(Value: TAlignment);
begin
...
procedure TvCaptionButton.SetCaption(Value: string);
begin
...
procedure TvCaptionButton.SetDown(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetEnabled(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetFont(Value: TFont);
begin
  if FFont <> Value then
  begin
    FFont.Assign(Value);
...
{$IFDEF JVCLThemesEnabled}
procedure TvCaptionButton.SetForceDrawSimple(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetHeight(Value: Integer);
begin
...
procedure TvCaptionButton.SetImageIndex(const Value: TImageIndex);
begin
...
procedure TvCaptionButton.SetImages(const Value: TCustomImageList);
begin
...
procedure TvCaptionButton.SetLayout(const Value: TvCaptionButtonLayout);
begin
...
procedure TvCaptionButton.SetLeft(Value: Integer);
begin
  if FLeft <> Value then
  begin
    FLeft := Value;
...
procedure TvCaptionButton.SetMargin(const Value: Integer);
begin
...
procedure TvCaptionButton.SetMouseInControl(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetParentShowHint(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetPosition(const Value: Integer);
begin
...
procedure TvCaptionButton.SetShowHint(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetSpacing(const Value: Integer);
begin
...
procedure TvCaptionButton.SetStandard(Value: TJvStandardButton);
{$IFDEF JVCLThemesEnabled}
var
...
procedure TvCaptionButton.SetToggle(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetTop(Value: Integer);
begin
  if FTop <> Value then
  begin
    FTop := Value;
...
procedure TvCaptionButton.SetVisible(const Value: Boolean);
begin
...
procedure TvCaptionButton.SetWidth(Value: Integer);
begin
...
procedure TvCaptionButton.UnHook;
var
  P: TCustomForm;
begin
  P := ParentForm;
...
procedure TvCaptionButton.UpdateButtonRect(Wnd: THandle);
var
  WindowRect: TRect;
...
function TvCaptionButton.WndProcAfter(var Msg: TMessage): Boolean;
begin
...
function TvCaptionButton.WndProcBefore(var Msg: TMessage): Boolean;
begin
  case Msg.Msg of
    CM_SHOWHINTCHANGED:
      begin
...
//=== { TvCaptionButtonActionLink } =========================================

procedure TvCaptionButtonActionLink.AssignClient(AClient: TObject);
begin
  FClient := AClient as TvCaptionButton;
end;

function TvCaptionButtonActionLink.IsCaptionLinked: Boolean;
begin
  Result := inherited IsCaptionLinked and
    AnsiSameText(FClient.Caption, (Action as TCustomAction).Caption);
end;

function TvCaptionButtonActionLink.IsEnabledLinked: Boolean;
begin
  Result := inherited IsEnabledLinked and
    (FClient.Enabled = (Action as TCustomAction).Enabled);
end;

function TvCaptionButtonActionLink.IsHintLinked: Boolean;
begin
  Result := inherited IsHintLinked and
    (FClient.Hint = (Action as TCustomAction).Hint);
end;

function TvCaptionButtonActionLink.IsImageIndexLinked: Boolean;
begin
  Result := inherited IsImageIndexLinked and
    (FClient.ImageIndex = (Action as TCustomAction).ImageIndex);
end;

function TvCaptionButtonActionLink.IsOnExecuteLinked: Boolean;
begin
  Result := inherited IsOnExecuteLinked and
    (@FClient.OnClick = @Action.OnExecute);
end;

function TvCaptionButtonActionLink.IsVisibleLinked: Boolean;
begin
  Result := inherited IsVisibleLinked and
    (FClient.Visible = (Action as TCustomAction).Visible);
end;

procedure TvCaptionButtonActionLink.SetCaption(const Value: string);
begin
  if IsCaptionLinked then
    FClient.Caption := Value;
end;

procedure TvCaptionButtonActionLink.SetEnabled(Value: Boolean);
begin
  if IsEnabledLinked then
    FClient.Enabled := Value;
end;

procedure TvCaptionButtonActionLink.SetHint(const Value: string);
begin
  if IsHintLinked then
    FClient.Hint := Value;
end;

procedure TvCaptionButtonActionLink.SetImageIndex(Value: Integer);
begin
  if IsImageIndexLinked then
    FClient.ImageIndex := Value;
end;

procedure TvCaptionButtonActionLink.SetOnExecute(Value: TNotifyEvent);
begin
  if IsOnExecuteLinked then
    FClient.OnClick := Value;
end;

procedure TvCaptionButtonActionLink.SetVisible(Value: Boolean);
begin
