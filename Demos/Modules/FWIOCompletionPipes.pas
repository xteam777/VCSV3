...
type
  ...
  TFWPipeData = record
    ...
    ReadBuff: array [0..MAXWORD - 1] of AnsiChar;
    ReadBuffSize: DWORD;
    WriteBuff: array [0..MAXWORD - 1] of AnsiChar;
    WriteBuffSize: DWORD;
    UserData: Pointer;
  end;
    ...
    try
      ...
      try
        ...
        if NewPipeHandle = INVALID_HANDLE_VALUE then
          xLog('Can''t create pipe instance error code = ' + IntToStr(GetLastError))
        else
        begin
...
begin
  ...
  begin
    ...
    case Index of
      WAIT_FAILED:
        xLog('Pipe DoWork state WAIT_FAILED');
      WAIT_TIMEOUT:
      begin
        DoIdle;
        Continue;
      ...
      begin
        ...
        begin
          if not GetOverlappedResult(NewPipeHandle,
            ...
            xLog('Pipe GetOverlappedResult failed')
          else
            DoConnect(NewPipeHandle);
        end;
        PendingIO := CreatePipeInstance(NewPipeHandle, ConnectOverlapped);
...
begin
  ...
  begin
    ...
    begin
      ...
      if FEvent = 0 then
        xLog('Set active pipe error code = ' + IntToStr(GetLastError));
      FActive := True;
      DoWork;
    end;
  end
...
begin
  ...
  begin
    ...
    FEvent := CreateEvent(nil, True, True, nil);
    if FEvent = 0 then
      xLog('Can''t create pipe error code = ' + IntToStr(GetLastError));
