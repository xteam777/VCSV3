...
var
  S_RTCP_ERROR_CONNECT: String = 'Сервер недоступен.';
  S_RTCP_ERROR_NOCRYPT: String = 'Сервер не поддерживает шифрование.';
  S_RTCP_ERROR_NEEDCRYPT: String = 'Сервер требует шифрование.';
  S_RTCP_ERROR_WRONGCRYPT: String = 'Неправильный код безопасности.';
  S_RTCP_ERROR_INVALID: String = 'Неверный ответ от сервера (code %s).';
  S_RTCP_ERROR_REJECTED: String = 'Ответ от сервера отклонен.';
...
//    procedure SetAccountUserName(const Value: String);
//    procedure SetAccountPassword(const Value: String);
//    procedure SetAuthentificated(const Value: boolean);
...
//    property AccountUserName: String read FAccountUserName write SetAccountUserName;
//    property AccountPassword: String read FAccountPassword write SetAccountPassword;
//
//    property Authentificated: boolean read FAuthentificated write SetAuthentificated;
  end;
...
procedure TRtcPortalCli.PingTimerOnTimer(Sender: TObject);
begin
  SendPing(nil);
end;
...
//  FAuthentificated := False;
  ...
  FPingTimer.Interval := 5000;
  FPingTimer.OnTimer := PingTimerOnTimer;
...
//procedure TRtcPortalCli.SetAccountUserName(const Value: String);
//begin
//  if Value <> FAccountUserName then
//  begin
////    LogOut;
//    FAccountUserName := Trim(Value);
////    Prepare;
//  end;
//end;
//
//procedure TRtcPortalCli.SetAccountPassword(const Value: String);
//begin
//  if Value <> FAccountPassword then
//  begin
////    LogOut;
//    FAccountPassword := Trim(Value);
////    Prepare;
//  end;
//end;
//
//procedure TRtcPortalCli.SetAuthentificated(const Value: boolean);
//begin
//  if Value <> FAuthentificated then
//  begin
////    LogOut;
//    FAuthentificated := Value;
////    Prepare;
//  end;
//end;
