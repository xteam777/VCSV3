...
TWebPDecodeBGRAIntoFunc = function (const data: PByte; data_size: size_t;
...
class var
  ...
  WebPDecodeBGRAInto : TWebPDecodeBGRAIntoFunc;
...
public
  ...
  class function CompressImage(const ScreenData : PByte;
    ScreenStride: Integer; var ImageRect : TRect;
...
class procedure TRtcWebPCodec.Create;
begin
  ...
  WebPDecodeBGRAInto := GetProcAddress(DLL, PAnsiChar('WebPDecodeBGRAInto'));
...
class procedure TRtcWebPCodec.Destroy;
begin
  FreeLibrary(DLL);
end;

class procedure TRtcWebPCodec.SetComprParams(Quality : Single; ComprMethod : Integer);
begin
  Cfg.quality := Quality;
  Cfg.method := ComprMethod;
end;

class function TRtcWebPCodec.CompressImage(const ScreenData : PByte;
  ScreenStride: Integer; var ImageRect : TRect;
  var Output : PByte; var MaxOutputSize : Integer) : Integer;
//var
//    f : TFileStream;
var
  SBPos, ScreenStrideOfs : Cardinal;
  SBP : PByte;
  i, j : Integer;
begin
  ...
  // Нужно сделать так, чтобы указатель на первый байт изображения в ScreenBuff
  // был кратен WEBP_ALIGN_CST, для этого увелеичиваем изображение смещая
  // сначала одну координату Left или Top ближе к началу экрана, а затем вторую
  // В результате работы кодируемая часть экрана может увеличиться,
  // ее координаты сохраняться в ImageRect

   //!!!!!!!!!!!!!!!!
 //  Debug.Log('WebP ComprImage');
   SBPos := Cardinal(ScreenData) + ImageRect.Top * ScreenStride + (ImageRect.Left shl 2);
   {

  with ImageRect do
  begin
    SBPos := Cardinal(ScreenData) + Top * ScreenStride + (Left shl 2);
    if Width < Height then
    begin
      while (SBPos mod WEBP_ALIGN_CST <> 0) and (Top > 0) do
      begin
        Dec(SBPos, ScreenStride);
        Dec(Top);
      end;

      while (SBPos mod WEBP_ALIGN_CST <> 0) and (Left > 0) do
      begin
        Dec(SBPos, 4);
        Dec(Left);
      end;
  ...
  //Debug.Log('WebP Entering ImageRect (' + IntToStr(ImageRect.Left)
  //       + ', ' + IntToStr(ImageRect.Top) + ', ' + IntToStr(ImageRect.Right)
  //       + ', ' + IntToStr(ImageRect.Bottom) + ') Screen ' + IntToStr(SBPos) +
   //      ' Stride ' + IntToStr(ScreenStride));


 //Убираем альфу
 // Debug.Log('0');
 SBP := PByte(SBPos) + 3;
  ScreenStrideOfs := ScreenStride - (ImageRect.Width shl 2);
 // Debug.Log('1');
// Debug.Log('WebP ComprImage2 ' + IntToHex(Cardinal(ScreenData)) + ' '
 //  + IntToHex(SBPos) + ' ' + IntToHex(Cardinal(SBP)));
  for i := 0 to ImageRect.Height - 1 do
  begin
   // Debug.Log('WebP ComprImage3');
   for j := 0 to ImageRect.Width - 1 do
    begin
     // Debug.Log('WebP ComprImage40');
    // var bb : byte := SBP^;
     //Debug.Log('WebP ComprImage41');
    //   Debug.Log('4 ' + IntToStr(Integer(SBP)));

      SBP^ := 255;
     //  Debug.Log('WebP ComprImage5');

    //  Debug.Log('4');
      Inc(SBP, 4);
     // Debug.Log('5');
    end;
    Inc(SBP, ScreenStrideOfs);
  end;

   //Debug.Log('WebP InitInternal');

 // Debug.Log('WebP ComprImage6');
  ...
  begin
    ...
    argb := PCardinal(SBPos);
    argb_stride := ScreenStride shr 2; // in pixels
    memory_argb_ := PCardinal(SBPos);
  end;
  ...
  //  Debug.Log('WebP Encoding ImageRect (' + IntToStr(ImageRect.Left)
   //      + ', ' + IntToStr(ImageRect.Top) + ', ' + IntToStr(ImageRect.Right)
    //     + ', ' + IntToStr(ImageRect.Bottom) + ') ' + IntToHex(Cardinal(@WebPEncode)));
    Result := WebPEncode(@Cfg, @Pic);
    //Debug.Log('Encoded');

    if Result = 0 then
    begin
      Debug.Log('WebP Encoding Error ImageRect (' + IntToStr(ImageRect.Left)
         + ', ' + IntToStr(ImageRect.Top) + ', ' + IntToStr(ImageRect.Right)
         + ', ' + IntToStr(ImageRect.Bottom) + ')');
      exit; // error in WebPEncode
    end;//!!!! else Debug.Log('Done WebP Encode');
...
class procedure TRtcWebPCodec.DeCompressImage(const Data: PByte; DataSize: size_t;
                           Output: PByte; Size: size_t; Stride: Integer);
var
  p, p2 : pbyte;
  i, w, h : Integer;
...
begin
  ...
  WebPDecodeBGRAInto(Data, DataSize, Output, Size, Stride);
...
TRtcWebPCodec.Create;

finalization

TRtcWebPCodec.Destroy;
