...
uses
  ...
  rtcConn, rtcDataSrv, rtcSrvModule, DateUtils;
...
type
  ...
  protected
    ...
    procedure ClearUsers(); virtual; abstract;
    end;
  ...
  public
    ...
    procedure UserReLogin(Sender:TRtcConnection; const username,password:String; isService: Boolean; const ID:RtcString; uinfo:TRtcRecord);
    function UserLogin(Sender:TRtcConnection; const username,password:String; isService: Boolean; const ID:RtcString; uinfo:TRtcRecord):boolean;
...
//    procedure SQLDisconnect();
    ...
    procedure ForceMsgClear(funame: String);
    procedure ForceClearGet(uname: String);
    procedure ForceClearPut(uname: String);
  ...
  published
    ...
    property OnUserPing: TRtcPortalUserEvent read FOnUserPing write FOnUserPing;
    property OnUserPingTimeout: TRtcPortalUserEvent read FOnUserPingTimeout write FOnUserPingTimeout;
  end;
...
begin
  ...
  tLogout.FForceUserLogout := ForceUserLogout;
  tLogout.Resume;
end;

procedure TRtcPortalGateway.ForceMsgClear(funame: String);
 var
    a:integer;
    MGet:TRtcRecord;
    uname:string;
  ...
  CS.Acquire;
  try
    MessGetOld.isNull[funame]:=True;
    MessGetTick.isNull[funame]:=True;
    MessPut.isNull[funame]:=True;
    MessPutTick.isNull[funame]:=True;
    MessPutDirect.isNull[funame]:=True;
    if MessGet.isType[funame]=rtc_Record then
      begin
      MGet:=MessGet.asRecord[funame];
      try
        MessGet.asObject[funame]:=nil;
...
procedure TRtcPortalGateway.ForceClearGet(uname: String);
  begin
  cs.Acquire;
  ClearWakeGet(uname);
  WakeGet(uname);
...
procedure TRtcPortalGateway.ForceClearPut(uname: String);
  begin
  cs.Acquire;
  ClearWakePut(uname);
  WakePut(uname);
...
procedure TRtcPortalGateway.ForceUserLogOut(uname: String);
  begin
  if WriteLog then xLog(uname + ': Logout check by ping timeout');
  // Log the user completely out ...
//  with TRtcDataServer(Sender) do
//    begin
    if UserLogout(nil, uname, '', True) then
    begin
      if WriteLog then
        xLog(uname + ': LOGGED OUT BY PING TIMEOUT');

      if Assigned(FOnUserLogout) then
        FOnUserLogout(uname);
//      DoTextEvent(TRtcConnection(Self), FOnUserLogout, uname);
    end;
//
...
procedure TRtcPortalGateway.ClearUserList();
  begin
    FUsers.ClearUsers;
  end;
...
begin
  ...
  try
    ...
    while i >= 0 do
    begin
     if LoggedIn.isType[LoggedIn.FieldName[i]] = rtc_Record then
        if (LoggedIn.asRecord[LoggedIn.FieldName[i]].asBoolean['IsService']
          and (IncSecond(LoggedIn.asRecord[LoggedIn.FieldName[i]].asDateTime['LastActiveTime'], FPingTimeout_Services) < Now))
          or (not LoggedIn.asRecord[LoggedIn.FieldName[i]].asBoolean['IsService']
            and (IncSecond(LoggedIn.asRecord[LoggedIn.FieldName[i]].asDateTime['LastActiveTime'], FPingTimeout_Users) < Now)) then
        begin
          if LoggedIn.FieldName[i] = '' then
            Continue;
...
function TRtcPortalGateway.UserLogin(Sender:TRtcConnection; const username, password:String; isService: Boolean; const ID: RtcString; uinfo:TRtcRecord):boolean;
  begin
  if username='' then
...
//      end;
    ...
    LoggedIn.asRecord[username].asBoolean['IsService'] := isService;
    LoggedIn.asRecord[username].asDateTime['LastActiveTime'] := Now;
  finally
    CS.Release;
    end;
  end;

procedure TRtcPortalGateway.UserReLogin(Sender:TRtcConnection; const username, password:String; isService: Boolean; const ID: RtcString; uinfo:TRtcRecord);
  begin
  CS.Acquire;
  try
    if UserLoggedIn(Sender,username,ID) then
      UserLogin(Sender,username,password,isService,ID,uinfo)
    else
            ...
            if list.isType[uname]<>rtc_Null then
              ...
              if WriteLog then
                if Assigned(Sender) then
                  xLog(username+' ['+Sender.PeerAddr+']: Stopped "'+func+'" with Control <'+uname+'>')
                else
                  xLog(username+': Stopped "'+func+'" with Control <'+uname+'>');
              //-sstuman
        ...
        if user.isType[func]=rtc_Record then
          ...
          for i:=0 to list.Count-1 do
            ...
            if list.isType[uname]<>rtc_Null then
              ...
              if WriteLog then
                if Assigned(Sender) then
                  xLog(username+' ['+Sender.PeerAddr+']: Terminated "'+func+'" from Host <'+uname+'>')
                else
                  xLog(username+' : Terminated "'+func+'" from Host <'+uname+'>');
              //-sstuman
...
//  if WriteLog then xLog(Param.asString['user']+' ['+Sender.PeerAddr+']: Re-Login check');
  ...
  UserReLogin(Sender,Param.asText['user'], Param.asText['pwd'], Param.asBoolean['iss'], Param.asString['id'], Param.asRecord['info']);
  with TRtcDataServer(Sender) do
    begin
...
procedure TRtcPortalGateway.FnLogInExecute(Sender: TRtcConnection; Param: TRtcFunctionInfo; Result: TRtcValue);
  ...
  if UserLogin(Sender,Param.asText['user'], Param.asText['pwd'], Param.asBoolean['iss'], Param.asString['id'], Param.asRecord['info']) then
    begin
...
//     begin
//      CS.Acquire;
//      dt := LoggedIn.asRecord[Session.asText['user']].asDateTime['LastActiveTime'];
//      CS.Release;
//
//      if IncSecond(dt, 10) < Now then //Доделать. Нужно проверять 'to'
//      begin
//        if WriteLog then xLog(Param.asString['user']+' ['+Sender.PeerAddr+']: Logout check');
//        // Log the user completely out ...
//        if UserLogout(Sender,Param.asText['user'], Param.asText['pwd'], True) then
...
//     end;
        ...
        //+sstuman
//        if not UserLoggedIn(Sender,Session.asText['user'],Session.asText['id']) then
//        begin
//          UserLogin(Sender,Session.asText['user'], Session.asText['pwd'], Session.asBoolean['iss'], Session.asString['id'], Session.asRecord['info']); // then
////              begin
//          if WriteLog then xLog(Session.asString['user']+' ['+Sender.PeerAddr+']: LOGGED IN');
//          DoTextEvent(Sender,FOnUserLogin,Session.asText['user']);
////              end;
//        end;
        //-sstuman
    ...
    if Param.isType['p']=rtc_DataSet then
      ...
      if CheckMsgPut(Session.asText['user'], Param.asInteger['x']) then
        ...
        begin
          CS.Acquire;
          try
            if LoggedIn.isType[Session.asText['user']] = rtc_Record then
            begin
//              DoTextEvent(Sender, FOnUserPing, 'Ping - ' + Session.asText['user'] + ' - ' + DateTimeToStr(Now));
              LoggedIn.asRecord[Session.asText['user']].asDateTime['LastActiveTime'] := Now;
            end;
          finally
            CS.Release;
          end;
